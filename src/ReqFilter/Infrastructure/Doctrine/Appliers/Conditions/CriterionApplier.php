...
namespace App\ReqFilter\Infrastructure\Doctrine\Appliers\Conditions;
...
final class CriterionApplier implements CriteriaApplierInterface
{
    public function apply(QueryBuilder $qb, string $alias, FilterDto $dto, int $countWhere): int {
        foreach ($dto->getConditions() as $group) {
            ...
            $countWhere = $this->canBeIn($group) ? $this->applyIn($qb, $alias, $group, $countWhere) : $this->applyGrouped($qb, $alias, $group, $countWhere);
        }
    ...
    private function canBeIn(ConditionGroup $group): bool
    {
        ...
        foreach ($group->conditions as $condition)
            if (!$condition instanceof Criterion || $condition->operator !== ComparisonOperator::EQUAL) return false;
        return true;
    }
